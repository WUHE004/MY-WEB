...
import { Database, Search, Plus, Trash2, Edit3, ChevronUp, ChevronDown, ChevronLeft, ChevronRight, Loader2, X, HardDrive, Image, FolderOpen, RefreshCw } from "lucide-react";
...
export function DbAdminPanel() {
  ...
  const fetchStorageFiles = useCallback(async (bucket: string, path: string) => {
    setStorageLoading(true);
    setStorageError("");
    try {
      const res = await fetch(`/api/db-admin/storage?bucket=${encodeURIComponent(bucket)}&path=${encodeURIComponent(path)}`);
      const json = await res.json();
      if (json.error) { setStorageError(json.error); } else { setStorageFiles(json.files || []); }
  ...
  const handleSelectBucket = (bucketName: string) => {
    ...
    setCurrentPath("");
    fetchStorageFiles(bucketName, "");
  };

  const handleOpenFolder = (folderName: string) => {
    const newPath = currentPath ? `${currentPath}/${folderName}` : folderName;
    setCurrentPath(newPath);
    fetchStorageFiles(selectedBucket, newPath);
  };
  ...
  const handleDeleteFile = async (fileName: string) => {
    ...
    try {
      ...
      const json = await res.json();
      if (json.error) { alert("删除失败: " + json.error); } else { fetchStorageFiles(selectedBucket, currentPath); }
    } catch { alert("删除失败"); }
  };
  ...
  return (
    <div className="flex gap-0 -mx-4 sm:-mx-6 lg:-mx-10 xl:-mx-14 h-[calc(100vh-260px)] min-h-[500px]">
      {/* ===== 左侧导航 ===== */}
      <div className="w-48 lg:w-56 shrink-0 border-r-[3px] border-gray-200 bg-gray-50 flex flex-col rounded-l-2xl">
        {/* 功能切换 */}
        <div className="p-3 border-b-2 border-gray-200">
          <div className="flex items-center gap-2 mb-2">
            <Database className="h-4 w-4 text-gray-500" />
            <span className="text-xs font-extrabold text-gray-500">数据管理</span>
          </div>
          <div className="flex gap-1.5">
            <button
              ...
              className={`flex-1 py-1.5 text-[10px] font-extrabold rounded-lg border-[2px] transition-colors ${
                activePanel === "database" ? "border-gray-900 bg-gray-900 text-white" : "border-gray-200 bg-white text-gray-500 hover:border-gray-400"
              }`}
            >
              数据库
            </button>
            <button
              ...
              className={`flex-1 py-1.5 text-[10px] font-extrabold rounded-lg border-[2px] transition-colors ${
                activePanel === "storage" ? "border-gray-900 bg-gray-900 text-white" : "border-gray-200 bg-white text-gray-500 hover:border-gray-400"
              }`}
            >
            ...
            <div className="p-3 border-b-2 border-gray-200">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="搜索表名..." className="w-full pl-7 pr-2 py-1.5 text-xs rounded-lg border-[2px] border-gray-200 focus:border-gray-900 focus:outline-none bg-white"
                />
              </div>
            </div>
            <div className="flex-1 overflow-y-auto">
              {tableLoading ? (
                ...
                filteredTables.map((table) => (
                  <button
                    key={table.name} onClick={() => handleSelectTable(table)}
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto">
              {storageLoading ? (
                ...
                buckets.map((bucket) => (
                  <button
                    key={bucket.id} onClick={() => handleSelectBucket(bucket.name)}
                    className={`w-full text-left px-3 py-2 text-xs font-bold transition-colors border-b border-gray-100 ${
      ...
      {/* ===== 右侧内容区 ===== */}
      <div className="flex-1 flex flex-col overflow-hidden bg-white rounded-r-2xl border-[3px] border-gray-200 border-l-0">
        {/* ===== 数据库模式 ===== */}
        {activePanel === "database" && (
          !selectedTable ? (
            ...
            <>
              {/* 工具栏 */}
              <div className="flex items-center gap-3 p-3 border-b-2 border-gray-200 shrink-0 flex-wrap">
                ...
                <button onClick={handleAdd} className="flex items-center gap-1 px-3 py-1.5 text-xs font-bold rounded-xl border-[3px] border-gray-900 bg-white hover:bg-gray-50 transition-colors">
                    ...
                    <tbody>
                      {data.map((row, idx) => (
                        <tr key={row.id as string || idx} className="border-b border-gray-100 hover:bg-gray-50">
                          {getDisplayColumns().map((col) => {
                            const val = row[col.name];
                            const isPhoto = col.isPhoto || (col.name === "photo" || col.name === "photo_url" || col.name === "url");
                            ...
                            return (
                              <td key={col.name} className="py-1 px-2 text-gray-700 max-w-[180px] align-top">
                                {imgUrl ? (
                                  <div>
                                    <img src={imgUrl} alt="" className="w-16 h-16 object-cover rounded-lg border border-gray-200 mb-0.5"
                                      onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                                    <div className="text-[9px] text-gray-400 truncate max-w-[150px]" title={imgUrl}>
                                      {imgUrl.length > 30 ? imgUrl.substring(0, 30) + "..." : imgUrl}
                                    </div>
                                  </div>
                                ) : (
                                  <span className="text-[11px] leading-tight break-all" title={formatCell(val)}>
                                    {formatCell(val)}
                                  </span>
                                )}
                              </td>
              ...
              {/* 分页 */}
              <div className="flex items-center justify-between p-3 border-t-2 border-gray-200 shrink-0">
          ...
          !selectedBucket ? (
            ...
            <>
              ...
              <div className="flex items-center gap-3 p-3 border-b-2 border-gray-200 shrink-0 flex-wrap">
                ...
                {currentPath && (
                  <span className="text-[10px] text-gray-400">/{currentPath}</span>
                )}
                <div className="flex-1" />
                <button onClick={() => fetchStorageFiles(selectedBucket, currentPath)}
                  className="flex items-center gap-1 px-2 py-1 text-[10px] font-bold rounded-lg border-[2px] border-gray-200 hover:bg-gray-50">
                  <RefreshCw className="h-3 w-3" />刷新
                </button>
              </div>
                    ...
                    <tbody>
                      {currentPath && (
                        <tr className="border-b border-gray-100 hover:bg-gray-50 cursor-pointer" onClick={handleGoBack}>
                          <td className="py-2 px-3 font-bold text-gray-500" colSpan={4}>
                        ...
                        <tr key={file.name} className="border-b border-gray-100 hover:bg-gray-50">
                          ...
                          <td className="py-1.5 px-3 text-right">
                            <button onClick={() => handleDeleteFile(file.name)}
                              className="p-1 rounded hover:bg-red-100 text-gray-500 hover:text-red-600" title="删除">
                              <Trash2 className="h-3 w-3" />
                            </button>
                          </td>
                        </tr>
